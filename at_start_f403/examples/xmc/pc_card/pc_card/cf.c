...
@endverbatim
  ...
  * @retval None
  */
void MYDMA_Config(dma_channel_type* DMAy_channelx, uint32_t cpar_src, uint32_t cmar_dst, uint32_t cndtr, uint8_t Transfer_Type, uint8_t Transfer_Width)
{
  dma_init_type  DMA_InitStruct;
  ...
  {
    DMA_InitStruct.peripheral_inc_enable = FALSE;
    DMA_InitStruct.memory_inc_enable = TRUE;
  }
  else
  {
    DMA_InitStruct.peripheral_inc_enable = TRUE;
    DMA_InitStruct.memory_inc_enable = FALSE;
  }

  if(Transfer_Width == Enable_8_bit_Transfer)
  {
    DMA_InitStruct.peripheral_data_width = DMA_PERIPHERAL_DATA_WIDTH_BYTE;
    DMA_InitStruct.memory_data_width = DMA_MEMORY_DATA_WIDTH_BYTE;
  }
  else
  {
    DMA_InitStruct.peripheral_data_width = DMA_PERIPHERAL_DATA_WIDTH_HALFWORD;
    DMA_InitStruct.memory_data_width = DMA_MEMORY_DATA_WIDTH_HALFWORD;
  }

  DMA_InitStruct.direction = DMA_DIR_MEMORY_TO_MEMORY;
  DMA_InitStruct.buffer_size = cndtr;
  DMA_InitStruct.peripheral_base_addr = cpar_src;

  DMA_InitStruct.memory_base_addr = cmar_dst;

  DMA_InitStruct.loop_mode_enable = FALSE;
  DMA_InitStruct.priority = DMA_PRIORITY_MEDIUM;
  dma_init(DMAy_channelx,&DMA_InitStruct);
 
  
  dma_channel_enable(DMAy_channelx, TRUE);
}
/**
  * @brief  XMC InitCtrl
...
StatusType XMC_PCCARD_InitCtrl(xmc_bank4_type *Device, xmc_pccard_init_type *Init, uint8_t Set_reg_8_bit)
{
  if(Set_reg_8_bit == 1)
  {
   Device->bk4ctrl_bit.extmdbw = 0x0;
  }
  else
  {
   Device->bk4ctrl_bit.extmdbw = 0x1;
  }
...
StatusType PCCARD_Init(PCCARD_HandleType *hpccard, xmc_nand_pccard_timinginit_type *ComSpaceTiming, xmc_nand_pccard_timinginit_type *AttSpaceTiming, xmc_nand_pccard_timinginit_type *IOSpaceTiming)
{
 ...
 if(hpccard->CF.Enable_8_bit_mode == TRUE)
  ...
  /* address line:PF0 PF1 PF2 PF3 PF4 PF5 PF12 PF13 PF14 PF15 PG0 */
  ...
  /* data line:   PD14 PD15 PD0 PD1 PE7 PE8 PE9 PE10 PE11 PE12 PE13 PE14 PE15 PD8 PD9 PD10 */

  gpio_default_para_init(&GPIO_InitStructure);
  GPIO_InitStructure.gpio_pins = GPIO_PINS_14|GPIO_PINS_15|GPIO_PINS_0|GPIO_PINS_1|GPIO_PINS_8|GPIO_PINS_9|GPIO_PINS_10; 
  GPIO_InitStructure.gpio_drive_strength = GPIO_DRIVE_STRENGTH_MODERATE;
  GPIO_InitStructure.gpio_mode = GPIO_MODE_MUX;
  GPIO_InitStructure.gpio_out_type = GPIO_OUTPUT_PUSH_PULL;
  GPIO_InitStructure.gpio_pull = GPIO_PULL_NONE;
  ...
  gpio_default_para_init(&GPIO_InitStructure);
  GPIO_InitStructure.gpio_pins = GPIO_PINS_11|GPIO_PINS_12|GPIO_PINS_13|GPIO_PINS_14|GPIO_PINS_15|GPIO_PINS_7|GPIO_PINS_8|GPIO_PINS_9|GPIO_PINS_10; 
  GPIO_InitStructure.gpio_drive_strength = GPIO_DRIVE_STRENGTH_MODERATE;
  GPIO_InitStructure.gpio_mode = GPIO_MODE_MUX;
  GPIO_InitStructure.gpio_out_type = GPIO_OUTPUT_PUSH_PULL;
  GPIO_InitStructure.gpio_pull = GPIO_PULL_NONE;
  ...
  /* XMC_NCE4_1/XMC_NCE4_2:PG10/PG11 XMC_NOE:PD4 XMC_NWE:PD5  XMC_NWAIT:PD6 */
  /* XMC_NIORD:PF6  XMC_NIOWR:PF8   XMC_NREG:PF7   XMC_INTR:PF10  XMC_CD:PF9  XMC_NIOS16:PF11 */

  gpio_default_para_init(&GPIO_InitStructure);
  GPIO_InitStructure.gpio_pins = GPIO_PINS_4|GPIO_PINS_6|GPIO_PINS_5; 
  GPIO_InitStructure.gpio_drive_strength = GPIO_DRIVE_STRENGTH_MODERATE;
  GPIO_InitStructure.gpio_mode = GPIO_MODE_MUX;
  GPIO_InitStructure.gpio_out_type = GPIO_OUTPUT_PUSH_PULL;
  ...
  gpio_default_para_init(&GPIO_InitStructure);
  GPIO_InitStructure.gpio_pins = GPIO_PINS_9|GPIO_PINS_6|GPIO_PINS_7|GPIO_PINS_8|GPIO_PINS_10|GPIO_PINS_11; 
  GPIO_InitStructure.gpio_drive_strength = GPIO_DRIVE_STRENGTH_MODERATE;
  GPIO_InitStructure.gpio_mode = GPIO_MODE_MUX;
  GPIO_InitStructure.gpio_out_type = GPIO_OUTPUT_PUSH_PULL;
  GPIO_InitStructure.gpio_pull = GPIO_PULL_NONE;
...
uint8_t *Read_Sector(PCCARD_HandleType *hpccard, uint8_t *pBuffer, uint32_t Transfer_Size)
{
  ...
  if(hpccard->CF.DMAEnable == TRUE)
  {
    ...
    {
      MYDMA_Config(DMA1_CHANNEL1, (uint32_t)hpccard->CF.IOAddr, (uint32_t)pBuffer, Transfer_Size, Read_Transfer, Enable_8_bit_Transfer);
    }
    else
    {
      MYDMA_Config(DMA1_CHANNEL1, (uint32_t)hpccard->CF.IOAddr, (uint32_t)pBuffer, (Transfer_Size / 2), Read_Transfer, Enable_16_bit_Transfer);
    }
...
uint8_t *Write_Sector(PCCARD_HandleType *hpccard, uint8_t *pBuffer, uint32_t Transfer_Size)
{
  ...
  if(hpccard->CF.DMAEnable == TRUE)
  {
    if(hpccard->CF.Enable_8_bit_mode)
    {
      MYDMA_Config(DMA1_CHANNEL1, (uint32_t)pBuffer, (uint32_t)hpccard->CF.IOAddr, Transfer_Size, Write_Transfer, Enable_8_bit_Transfer);
    }
    else
    {
      MYDMA_Config(DMA1_CHANNEL1, (uint32_t)pBuffer, (uint32_t)hpccard->CF.IOAddr, (Transfer_Size / 2), Write_Transfer, Enable_16_bit_Transfer);
    }
